...
func (c *Command) sessionCmd() *cobra.Command {
	...
	cmd.AddCommand(c.sessionDeleteCmd())
	return cmd
}
...
func (c *Command) sessionDeleteCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			resp, err := cc.Client.DeleteSession(cc.Ctx, &proto.DeleteSessionRequest{
				SessionId: sessionID,
			})
			if err != nil {
				log.Fatal().Err(err).Msg("failed to delete session")
			}

			if resp.Success {
				log.Info().Str("session_id", sessionID).Msg(resp.Message)
			} else {
				log.Error().Str("session_id", sessionID).Msg(resp.Message)
			}
		},
	}
}
